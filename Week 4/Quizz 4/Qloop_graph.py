...
for dim in d:
	...
	if dim < 2: 
		V_sph_1 = 2
		if dim == 1:
			V_list.append(V_sph_1)
			V_list_teo.append(V_sph(1))
	else:
		dimensions = dim - 1
		#Coordinates vector
		x = []
		...
		V_list.append(V_sph_1*reduce(lambda x,y:x*y,Q_list))
		V_list_teo.append(V_sph(d[dim]))

# graphics output
plt.plot(range(1,d_max+1), V_list, color='black', label='markov')
plt.plot(range(1,d_max+1), V_list_teo,'o', color='blue', label='exact',markevery=5)
plt.legend()
plt.title('Sampling of the Volumes')
plt.yscale('log')
plt.xlabel('$d$', fontsize=14)
plt.ylabel('$V_sph(d)$', fontsize=14)
plt.savefig('V_sph.png',dpi=200)
